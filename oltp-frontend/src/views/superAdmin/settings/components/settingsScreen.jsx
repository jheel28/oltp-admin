...
const SettingsScreen = () => {
  ...
  const handleProfileImageUpdate = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/beta/superadmin/update/image/byid/${auth.userId}`,
        {
  ...
  const handlePasswordChange = async () => {
    if (passwordMatch) {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/beta/superadmin/update/password/byemail/${auth.email}`,
          {
            method: "PATCH",
            headers: {
        ...
        if (response.status === 200) {
          message.success("Super admin password updated successfully");
          setTimeout(() => {
            window.location.reload();
          }, 2000);
        } else {
          message.error(
            "Could not update super admin password, please try again"
          );
        }
      } catch (err) {
        message.error(
