...
const Dashboard = () => {
  ...
  const avgScore =
    attempted.length > 0
      ? Math.round(
          attempted.reduce(
            (acc, t) => acc + (t.totalMarks > 0 ? (t.marksObtained / t.totalMarks) * 100 : 0),
            0
          ) / attempted.length
        )
      : 0;
          ...
          <div className="flex flex-col gap-3">
            {upcomingTests.length > 0 ? (
              upcomingTests.slice(0, 3).map((test) => (
                <TestCard key={test._id || test.testId} test={test} isActive={false} />
              ))
            ) : (
              <EmptyState
                message="No Upcoming Exams"
                sub="Exams assigned to your batch will appear here."
                icon={MdSchedule}
              />
            )}
          </div>
