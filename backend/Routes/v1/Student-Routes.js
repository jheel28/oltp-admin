...
const phoneValidation = (field) =>
  check(field)
    ...
    .matches(/^\+[1-9]\d{6,14}$/).withMessage(`${field} must be in international format (e.g. +919876543210)`);

const studentValidation = [
  check("firstName").trim().isLength({ min: 1, max: 255 }),
  check("lastName").trim().isLength({ min: 1, max: 255 }),
  phoneValidation("phoneNumber"),
  phoneValidation("alternateNumber"),
  ...
  check("fatherName").optional({ nullable: true, checkFalsy: true }).trim().isLength({ max: 255 }),
  check("motherName").optional({ nullable: true, checkFalsy: true }).trim().isLength({ max: 255 }),
];

const updateValidation = [
  check("firstName").optional().trim().isLength({ min: 1, max: 255 }),
  check("lastName").optional().trim().isLength({ min: 1, max: 255 }),
  check("phoneNumber").optional().trim().matches(/^\+[1-9]\d{6,14}$/).withMessage("phoneNumber must be in international format"),
  check("alternateNumber").optional().trim().matches(/^\+[1-9]\d{6,14}$/).withMessage("alternateNumber must be in international format"),
